...
export interface CodeComponentBuilder {
...
function getInputFiles(
  ...
  const inputFiles = Object.keys(metafile.inputs)
    .filter((inputPath) => inputPath !== entrySourceFile)
    .map((inputPath) =>
      inputPath.startsWith(`${sourceNamespace}:`)
        ? inputPath.slice(sourceNamespace.length + 1)
        : inputPath,
    )
    .map((inputPath) =>
      path.isAbsolute(inputPath)
        ? inputPath
        : path.resolve(projectDirectory, inputPath),
    )
    .filter(
      (inputPath) =>
        !path.normalize(inputPath).split(path.sep).includes("node_modules"),
    )
...
export function isCodeComponentBuildInput(
  buildResult: CodeComponentBuildResult,
  filePath: string,
): boolean {
  const absolutePath = path.resolve(filePath);

  if (
    [...buildResult.inputFiles].some(
      (inputPath) =>
        normalizeFilePath(inputPath) === normalizeFilePath(absolutePath),
    )
  ) {
    return true;
  }
  ...
  const relativePath = path
    .relative(buildResult.projectDirectory, absolutePath)
    .split(path.sep)
    .join("/");
...
function getModuleId(libraryName: string): string {
...
function resolveLocalImport(
  ...
  const unresolvedPath = importPath.startsWith("@/")
    ? path.join(projectDirectory, "src", importPath.slice(2))
...
function isProjectSource(projectDirectory: string, filePath: string): boolean {
...
function createBrowserSourcePlugin(projectDirectory: string): esbuild.Plugin {
  return {
    ...
    setup(build) {
      build.onResolve({ filter: /.*/ }, (args) => {
        if (
          ...
          isProjectSource(projectDirectory, args.path)
        ) {
          return { path: args.path, namespace: sourceNamespace };
        }

        if (
          args.namespace !== sourceNamespace ||
        ...
        return isProjectSource(projectDirectory, resolvedPath)
          ? { path: resolvedPath, namespace: sourceNamespace }
          : { path: resolvedPath };
      });
      ...
      build.onLoad(
        ...
        async (args) => ({
          ...
          loader: (path.extname(args.path) === ".mjs"
            ? "js"
            : path.extname(args.path).slice(1)) as esbuild.Loader,
  ...
  const componentImports = components
    .map(
      ({ path: componentPath }, index) =>
        `import component${index} from ${JSON.stringify(componentPath)};`,
    )
    .join("\n");
  const componentModules = components
    .map(
      ({ id }, index) =>
        `${JSON.stringify(`./${id}`)}: { default: component${index} },`,
    )
    .join("\n");
...
function createWebflowManifest(
  components: ComponentEntry[],
  publicPath: string,
...
function createFederationManifest(
  moduleId: string,
  components: ComponentEntry[],
  publicPath: string,
  cssFilename: string | undefined,
...
/**
 * Create an incremental builder for the project's Webflow Code Component
 * library. Returns undefined when the project has no library configuration.
 */
export async function createCodeComponentBuilder(
  ...
  if (!fs.existsSync(webflowConfigPath)) return undefined;
  ...
  if (
    ...
    !library.components.every((pattern) => typeof pattern === "string")
  ) {
  ...
  let componentEntrySignature: string | undefined;
  ...
  return {
    async build() {
      ...
      const cssOutput = Object.keys(metafile.outputs).find((file) =>
        file.endsWith(".css"),
      );
      const cssFilename = cssOutput ? path.basename(cssOutput) : undefined;
